...
class DefaultController extends Controller
{
    ...
    /**
     * @Route("/{id}/", defaults={"id" = 1})
     */
    public function indexActio(Service $s,Repository $r, $id)
    {
        $pieces = explode("-", $id);

        if (count($pieces)==2) {

            $res=$s->fromTo($pieces[0],$pieces[1]);

            return $this->render('index/table2.html.twig', array('r'=>$res));
        } else {
